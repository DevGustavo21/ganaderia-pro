...
export const listFarmMembers = async (farmId) => {
  ...
  const { data, error } = await supabase
    .from('farm_members')
    .select('role, joined_at, user_id, profiles:profiles!farm_members_user_id_fkey(id, full_name, email, avatar_url)')
    .eq('farm_id', farmId);
  if (error) throw error;
  return data ?? [];
};

export const inviteToFarm = async ({ farmId, email, role = 'worker', message }) => {
  const supabase = await supabaseServer();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('No autenticado');
  const { data, error } = await supabase
    .from('farm_invitations')
    .insert({ farm_id: farmId, email, role, message, invited_by: user.id })
    .select('*')
    .single();
  if (error) throw error;
  return data;
