...
import { GP, STATUS } from '@/lib/theme';
...
export const AnimalAvatar = ({ animal, size = 44 }) => {
...
export const Pill = ({ active, onClick, children, color, size = 'md', icon, style = {} }) => {
  const palette = {
...
export const Button = ({ children, onClick, variant = 'primary', size = 'md', icon, disabled, type = 'button', style = {}, fullWidth = false, className }) => {
  const variants = {
...
export const MetricChip = ({ label, value, sub, accent = GP.green }) => (
  <div style={{
    flex: '0 0 auto', minWidth: 116,
    padding: '10px 12px',
    background: GP.white, borderRadius: 12,
...
export const Field = ({ label, hint, required, children, error }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
    ...
    {error && (
      <div style={{ fontSize: 12, color: GP.red, display: 'flex', alignItems: 'center', gap: 4 }}>
...
export const Input = ({
  value, onChange, placeholder, type = 'text', large = false, prefix, suffix,
  autoComplete, name, id, style = {},
}) => (
  <div style={{
    ...
    {prefix && <span style={{ color: GP.textSec, fontSize: 14, fontWeight: 500, marginRight: 8, display: 'flex', alignItems: 'center' }}>{prefix}</span>}
    <input
      id={id}
      name={name}
      type={type}
    ...
    />
    {suffix && <span style={{ color: GP.textSec, fontSize: 13, fontWeight: 600, marginLeft: 8, display: 'flex', alignItems: 'center' }}>{suffix}</span>}
  </div>
);
